...
BASE_URL = os.environ.get('MINIMAX_BASE_URL', 'https://api.minimaxi.com').rstrip('/v1').rstrip('/')
...
def text_to_speech(text: str, voice_id: str = 'female-tianmei', speed: float = 1.0, output_path: str = None) -> dict:
    ...
    if output_path is None:
        # 默认保存到 workspace 的 media 目录
        media_dir = os.path.expanduser('~/.openclaw/workspace/media')
        if not os.path.exists(media_dir):
            os.makedirs(media_dir)
        output_path = os.path.join(media_dir, tempfile.mktemp(suffix='.mp3').split('/')[-1])
